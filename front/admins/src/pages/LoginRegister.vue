...
const canSubmit = computed(() => {
  ...
  return mode.value !== 'register' || (confirmPassword.value.trim() && password.value === confirmPassword.value)
})
...
function handleSubmit() {
  ...
  loading.value = true
  emit('login', { username: username.value.trim(), password: password.value, role: role.value })
          ...
          <form class="auth-form" @submit.prevent="handleSubmit">
            ...
            <transition name="form-slide">
              <div v-if="mode === 'register'" class="register-fields">
                ...
                <label class="role-select"><span>注册身份</span><select v-model="role"><option v-for="item in roleOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
              </div>
            </transition>
