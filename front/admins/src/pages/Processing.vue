...
const batchStatusLabels = ['', '加工中', '加工完成', '已废弃']
const shiftLabels = ['', '早班', '中班', '晚班']
const templateStatusLabels = ['', '启用', '停用']
...
async function submitProcess() {
  try {
    ...
    if (editing.value) {
      data.processBatchId = editing.value.processBatchId
      await productionApi.updateProcessBatch(data)
      flash('success', '加工批次更新成功')
    } else {
      await productionApi.createProcessBatch(data)
      flash('success', '加工批次创建成功')
    }
...
async function doDeleteProcess() {
  try {
    await productionApi.deleteProcessBatch(deletingId.value!)
    flash('success', '加工批次删除成功')
...
async function completeProcess(id: number) {
  try {
    await productionApi.completeProcessBatch(id)
    flash('success', '加工批次已完成')
    loadProcess()
  } catch (e: any) { flash('error', '操作失败: ' + e.message) }
}
...
async function submitTemplate() {
  try {
    ...
    if (editingTemplate.value) {
      data.templateId = editingTemplate.value.templateId
      await productionApi.updateTemplate(data)
      flash('success', '工艺模板更新成功')
    } else {
      await productionApi.createTemplate(data)
      flash('success', '工艺模板创建成功')
    }
...
function switchTab(t: 'process' | 'template') {
  tab.value = t
  if (t === 'process') loadProcess()
  else loadTemplates()
}
...
<template>
  <div class="page-module">
    ...
    <!-- Tabs -->
    <div class="tabs">
      <button class="tab-btn" :class="{ active: tab === 'process' }" @click="switchTab('process')">📋 加工批次</button>
    ...
    <template v-if="tab === 'process'">
      <div class="search-bar">
        ...
        <div class="search-field">
          <label>批次状态</label>
          <select v-model="filters.batchStatus"><option value="">全部</option><option value="1">加工中</option><option value="2">加工完成</option><option value="3">已废弃</option></select>
        </div>
        <div class="search-field">
          <label>班次</label>
      ...
      <div class="data-table-wrap">
        <table class="data-table">
          <thead>
            <tr>
              <th>加工批次号</th><th>产品名称</th><th>工艺模板</th><th>原料批次</th>
              <th>生产线</th><th>班次</th><th>计划数量</th><th>实际温度</th>
              <th>加工日期</th><th>状态</th><th class="col-actions">操作</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-for="row in list" :key="row.processBatchId">
              ...
              <td>{{ row.actualTemp || '-' }}</td>
              <td>{{ row.processDate }}</td>
              <td><span class="tag" :class="row.batchStatus === 1 ? 'tag-warn' : row.batchStatus === 2 ? 'tag-success' : 'tag-danger'">{{ batchStatusLabels[row.batchStatus] || '-' }}</span></td>
              <td class="col-actions">
                <button class="btn btn-outline btn-sm" @click="openEditProcess(row)">✎ 编辑</button>
    ...
    <template v-if="tab === 'template'">
      ...
      <div class="data-table-wrap">
        <table class="data-table">
          <thead>
            <tr>
              <th>模板名称</th><th>版本</th><th>适用产品</th><th>目标温度</th>
              <th>时长</th><th>压力</th><th>冷却温度</th><th>灌装温度</th>
              <th>搅拌速度</th><th>pH值</th><th>清洁等级</th><th>状态</th><th class="col-actions">操作</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-for="row in templates" :key="row.templateId">
              ...
              <td>{{ row.stirSpeed || '-' }}</td>
              <td>{{ row.phValue || '-' }}</td>
              <td>{{ row.cleanLevel }}</td>
              <td><span class="tag" :class="row.templateStatus === 1 ? 'tag-success' : 'tag-neutral'">{{ templateStatusLabels[row.templateStatus] || '-' }}</span></td>
              <td class="col-actions">
                <button class="btn btn-outline btn-sm" @click="openEditTemplate(row)">✎ 编辑</button>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
      <div class="modal" style="width:680px">
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="form-group"><label>工艺模板</label><input v-model="form.templateName" placeholder="关联工艺模板" /></div>
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label>操作员</label><input v-model="form.operator" placeholder="操作员" /></div>
          </div>
          <div class="form-row">
          ...
          <p style="margin:12px 0 8px;color:#6c84a3;font-size:13px;font-weight:700">实际工艺参数</p>
          <div class="form-row">
            <div class="form-group"><label>温度</label><input v-model="form.actualTemp" placeholder="℃" /></div>
            <div class="form-group"><label>时长</label><input v-model="form.actualDuration" placeholder="min" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>压力</label><input v-model="form.actualPressure" placeholder="MPa" /></div>
            <div class="form-group"><label>冷却温度</label><input v-model="form.actualCoolTemp" placeholder="℃" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>灌装温度</label><input v-model="form.actualFillTemp" placeholder="℃" /></div>
            <div class="form-group"><label>pH值</label><input v-model="form.actualPh" placeholder="pH" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>粘度</label><input v-model="form.actualViscosity" placeholder="mPa·s" /></div>
          </div>
    ...
    <div v-if="showTemplateModal" class="modal-overlay" @click.self="showTemplateModal = false">
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group"><label>模板名称 *</label><input v-model="templateForm.templateName" placeholder="模板名称" /></div>
            <div class="form-group"><label>版本</label><input v-model="templateForm.version" placeholder="V1.0" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>适用产品</label><input v-model="templateForm.applicableProduct" placeholder="适用产品" /></div>
            <div class="form-group"><label>目标温度</label><input v-model="templateForm.targetTemp" placeholder="℃" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>时长</label><input v-model="templateForm.duration" placeholder="min" /></div>
            <div class="form-group"><label>压力</label><input v-model="templateForm.pressure" placeholder="MPa" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>冷却温度</label><input v-model="templateForm.coolTemp" placeholder="℃" /></div>
            <div class="form-group"><label>灌装温度</label><input v-model="templateForm.fillTemp" placeholder="℃" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>搅拌速度</label><input v-model="templateForm.stirSpeed" placeholder="rpm" /></div>
            <div class="form-group"><label>pH值</label><input v-model="templateForm.phValue" placeholder="pH" /></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>粘度</label><input v-model="templateForm.viscosity" placeholder="mPa·s" /></div>
            <div class="form-group"><label>清洁等级</label><input v-model.number="templateForm.cleanLevel" type="number" placeholder="1-5" /></div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>模板状态</label>
          ...
          <div class="form-group"><label>备注</label><textarea v-model="templateForm.remark" placeholder="备注" /></div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-outline" @click="showTemplateModal = false">取消</button>
          <button class="btn btn-primary" @click="submitTemplate">{{ editingTemplate ? '保存' : '创建' }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showConfirm" class="confirm-overlay" @click.self="showConfirm = false">
      <div class="confirm-box">
        ...
        <h3>确认删除</h3>
        <p>删除后将不可恢复，确定要删除该加工批次吗？</p>
        <div class="confirm-actions">
          <button class="btn btn-outline" @click="showConfirm = false">取消</button>
          <button class="btn btn-danger" @click="doDeleteProcess">确认删除</button>
        </div>
      </div>
    </div>
  </div>
